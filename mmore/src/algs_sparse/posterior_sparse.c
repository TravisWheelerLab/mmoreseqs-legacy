                        ...
                        bool                    is_run_domains )  /* if run posterior on domains */  
{
   ...
   /* compute score and bias for entire cloud */
   {
      ...
      scores->sparse_bound_fwd   = fwd_sc;
      ...
      scores->sparse_bound_bck   = bck_sc;
   ...
   if ( is_run_domains == true )
   {
      ...
      D_total                 = 0;
      null1_hmm_bias          = dom_def->nullsc;
      dom_start_time          = CLOCK_GetTime( timer );
      ...
      for (int i = 0; i < N_domains; i++)
      {
         ...
         fprintf(stdout, "# ==> Null2 Compo Bias  (domain %d/%d): %11.4f %11.4f\n", 
            i+1, N_domains, null2_seq_bias, null2_seq_bias/CONST_LOG2);
      ...
      times->dom_total = CLOCK_GetTime( timer ) - dom_start_time;
   }
