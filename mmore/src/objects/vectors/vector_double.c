                  ...
                  VECTOR_DBL*   src )
{
   ...
   VECTOR_DBL_Resize( dest, src->N );
   /* copy variable-sized data */
   for (int i = 0; i < src->N; i++ ) {
      VEC_X( dest, i ) = DBL_Create( VEC_X( src, i ) );
   }
   /* copy base data */
   dest->N = src->N;
 ...
 */
int 
VECTOR_DBL_Search(   VECTOR_DBL*   vec, 
                     DBL           val )
{
 ...
 */
int 
VECTOR_DBL_Search_First(  VECTOR_DBL*   vec, 
                           DBL           val )
{
   int N       = VECTOR_DBL_GetSize( vec );
   int idx     = (N/2);
 ...
 */
int 
VECTOR_DBL_Search_Last(    VECTOR_DBL*   vec, 
                           DBL           val )
{
   int N       = VECTOR_DBL_GetSize( vec );
   int idx     = (N/2);
 ...
 */
int 
VECTOR_DBL_Compare(  VECTOR_DBL*   vec_A, 
                     VECTOR_DBL*   vec_B )
{
 ...
 */
inline
STATUS_FLAG VECTOR_DBL_Reverse(   VECTOR_DBL*    vec )
{
   int N = VECTOR_DBL_GetSize( vec );
...
/*! FUNCTION:  VECTOR_DBL_LoadTSV()
 *  SYNOPSIS:  Load tsv from <filename> and store in <vec>.
 */
STATUS_FLAG 
VECTOR_DBL_LoadTSV(  VECTOR_DBL*    vec,
                     char*          filename )
{
