 ...
 */
XXX* 
VECTOR_XXX_UnwrapArray(    VECTOR_XXX*    vec,
                           size_t         size )
{
   XXX* array = vec->data;
   vec = ERROR_malloc( sizeof(VECTOR_XXX) );
 ...
 */
int 
VECTOR_XXX_Search(   VECTOR_XXX*   vec, 
                     XXX           val )
{
   int N       = VECTOR_XXX_GetSize( vec );
   int idx     = (N/2);
 ...
 */
int 
VECTOR_XXX_Search_First(  VECTOR_XXX*   vec, 
                           XXX           val )
{
   int N       = VECTOR_XXX_GetSize( vec );
   int idx     = (N/2);
 ...
 */
int 
VECTOR_XXX_Search_Last(    VECTOR_XXX*   vec, 
                           XXX           val )
{
   int N       = VECTOR_XXX_GetSize( vec );
   int idx     = (N/2);
 ...
 */
int 
VECTOR_XXX_Compare(  VECTOR_XXX*   vec_A, 
                     VECTOR_XXX*   vec_B )
{
 ...
 */
inline
STATUS_FLAG VECTOR_XXX_Reverse(   VECTOR_XXX*    vec )
{
   int N = VECTOR_XXX_GetSize( vec );
                        ...
                        FILE*          fp )
{
   ...
   fprintf(fp, "[ ");
   for ( int i = 0; i < vec->N; i++ ) {
      fprintf(fp, "%s%s%s", XXX_To_String(vec->data[i], s), delim, pad );
   }
   fprintf(fp, "]\n" );
}
