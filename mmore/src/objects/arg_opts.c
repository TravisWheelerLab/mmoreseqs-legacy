...
{
   if ( arg_opts == NULL ) return NULL;

   /* general data */
   arg_opts->opt_names  = VECTOR_STR_Create( arg_opts->opt_names );
   arg_opts->opt_desc   = VECTOR_STR_Create( arg_opts->opt_desc );
   arg_opts->opt_help   = VECTOR_STR_Create( arg_opts->opt_help );
   /* option data */
   arg_opts->opt_long   = VECTOR_STR_Create( arg_opts->opt_long );
   arg_opts->opt_short  = VECTOR_STR_Create( arg_opts->opt_short );
   arg_opts->num_args   = VECTOR_INT_Create( arg_opts->num_args );
   arg_opts->arg_index  = VECTOR_INT_Create( arg_opts->arg_index );
   /* argument data */
   arg_opts->arg_type   = VECTOR_INT_Create( arg_opts->arg_type );
   arg_opts->arg_loc    = VECTOR_PTR_Create( arg_opts->arg_loc );
   arg_opts->arg_id     = VECTOR_INT_Create( arg_opts->arg_id );
 ...
 */
INT 
ARG_OPTS_Index(   ARG_OPTS*   arg_opts, 
                  int         opt_id )
{
 ...
 */
INT 
ARG_OPTS_GetOption_byName(    ARG_OPTS*   arg_opts, 
                              STR         opt_name )
{
   int   opt_id;
   int   N_opts;
