...
WORK_viterbi_test_threshold( WORKER*  worker )
{
   ...
   /* otherwise passes if eval is better (smaller) */
      ...
      result->is_passed_viterbi = ( vit_eval < vit_threshold );
   }
...
WORK_cloud_test_threshold( WORKER* worker )
{
   ...
   float    cloud_eval       = result->score_cloud;
   ...
   /* otherwise passes if eval is better (smaller) */
      ...
      result->is_passed_cloud = ( cloud_eval < cloud_threshold );
   }
...
WORK_bound_fwdback_test_threshold( WORKER* worker )
{
   ...
   float    fwdback_eval       = result->score_fwdback;
   ...
   /* otherwise passes if eval is better (smaller) */
      ...
      result->is_passed_fwdback = ( fwdback_eval < fwdback_threshold );
   }
