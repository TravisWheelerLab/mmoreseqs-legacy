...
def load_matrix(tsv_matrix):
    ...
    return m, n, NM_MX, SP_MX.T, trace
...
TRACE = []
for i in range(N):
    m, n, N_MX, S_MX, TR = load_matrix(tsv_matrix[i])
    NM_MX.append(N_MX)
    SP_MX.append(S_MX)
    TRACE.append(TR)
...
for i in range(N):
    ...
    if opts["-4"]:
        output_heatmap(
            title1, MAT_MX[i], INS_MX[i], DEL_MX[i], SP_MX[i], min_val, max_val, file, name)
    if opts["-tr"]:
        output_heatmap_trace(
            title2, MAT_MX[i], TRACE[i], min_val, max_val, file, name)
    next
