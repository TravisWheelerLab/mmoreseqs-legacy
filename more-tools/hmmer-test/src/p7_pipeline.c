...
{
   printf("=> p7_pli_NewModelThresholds\n");
...
p7_Pipeline_TEST(P7_PIPELINE *pli, P7_OPROFILE *om, P7_BG *bg, const ESL_SQ *sq, const ESL_SQ *ntsq, P7_TOPHITS *hitlist, /* DAVID RICH EDIT */ P7_PROFILE *gm)
{
   ...
   if (pli->do_biasfilter)
   {
      ...
      // printf("=== IN p7_bg_FilterScore ===\n");
      // p7_bg_Dump(stdout, bg);
      // my_hmm_Dump(bg->fhmm, stdout);
      // printf("bg->fhmm->M: %d, bg->p1 = %f\n", bg->fhmm->M, bg->p1);
   ...
   P = exp(lnP) * pli->Z;
   printf("# PRESCORE: ln(P) = %9.9f, P = %9.9f = %4.3e\n",
      lnP, P, P);
   /* seqscore */
   lnP = esl_exp_logsurv (seq_score,  om->evparam[p7_FTAU], om->evparam[p7_FLAMBDA]);
   P = exp(lnP) * pli->Z;
   printf("# SEQSCORE: ln(P) = %9.9f, P = %9.9f = %4.3e\n",
      lnP, P, P);
   ...
   printf("# (final) seq_score: %.9f, pre_score: %.9f\n",
      seq_score, pre_score);
