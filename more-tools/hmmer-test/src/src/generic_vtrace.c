...
p7_GTrace(const ESL_DSQ *dsq, int L, const P7_PROFILE *gm, const P7_GMX *gx, P7_TRACE *tr)
{
   ...
   int state_num[] = {-1, p7G_M, p7G_D, p7G_I, -1, p7G_N, p7G_B, p7G_E, p7G_C, -1, p7G_J, -1};
   ...
   // char *is_local = p7_profile_IsLocal(gm) ? "is local" : "is NOT local";
   // printf("%s\n", is_local);
   ...
   while (sprv != p7T_S) {
      ...
      switch (sprv) 
      {
         case p7T_C:      /* C(i) comes from C(i-1) or E(i) */
            ...
            if      (esl_FCompare(XMX(i, p7G_C), XMX(i - 1, p7G_C) + gm->xsc[p7P_C][p7P_LOOP], tol) == eslOK)  
               ...
               ESL_EXCEPTION(eslFAIL, "C at i=%d couldn't be traced", i);
            break;
         ...
         case p7T_E:      /* E connects from any M state. k set here */
            ...
            if (p7_profile_IsLocal(gm))
            {
               ...
               for (k = M; k >= 1; k--) {
                  // printf("testing E at (%d, %d) => (%.2f v. %.2f) \n", i, k, XMX(i, p7G_E), MMX(i, k) );
                  if (esl_FCompare(XMX(i, p7G_E), MMX(i, k), tol) == eslOK) 
                     break;
               }
               if (k == 0) 
                  ESL_EXCEPTION(eslFAIL, "E at i=%d couldn't be traced", i);
            }
            else        /* glocal mode: we either come from D_M or M_M */
            {
               if      (esl_FCompare(XMX(i, p7G_E), MMX(i, M), tol) == eslOK) 
                  { scur = p7T_M; k = M; }
               else if (esl_FCompare(XMX(i, p7G_E), DMX(i, M), tol) == eslOK) 
                  { scur = p7T_D; k = M; }
               else    
                  ESL_EXCEPTION(eslFAIL, "E at i=%d couldn't be traced", i);
            }
         ...
         case p7T_M:         /* M connects from i-1,k-1, or B */
            ...
            if      (esl_FCompare(MMX(i, k), XMX(i - 1, p7G_B) + TSC(p7P_BM, k - 1) + MSC(k), tol) == eslOK) 
               scur = p7T_B;
            else if (esl_FCompare(MMX(i, k), MMX(i - 1, k - 1)   + TSC(p7P_MM, k - 1) + MSC(k), tol) == eslOK) 
               scur = p7T_M;
            else if (esl_FCompare(MMX(i, k), IMX(i - 1, k - 1)   + TSC(p7P_IM, k - 1) + MSC(k), tol) == eslOK) 
               scur = p7T_I;
            else if (esl_FCompare(MMX(i, k), DMX(i - 1, k - 1)   + TSC(p7P_DM, k - 1) + MSC(k), tol) == eslOK) 
               scur = p7T_D;
            else 
               ESL_EXCEPTION(eslFAIL, "M at k=%d,i=%d couldn't be traced", k, i);
            k--; i--;
            break;
         ...
         case p7T_D:         /* D connects from M,D at i,k-1 */
            ...
            if      (esl_FCompare(DMX(i, k), MMX(i, k - 1) + TSC(p7P_MD, k - 1), tol) == eslOK) 
               scur = p7T_M;
            else if (esl_FCompare(DMX(i, k), DMX(i, k - 1) + TSC(p7P_DD, k - 1), tol) == eslOK) 
               scur = p7T_D;
         ...
         case p7T_I:         /* I connects from M,I at i-1,k*/
            ...
            if      (esl_FCompare(IMX(i, k), MMX(i - 1, k) + TSC(p7P_MI, k) + ISC(k), tol) == eslOK) 
               scur = p7T_M;
            else if (esl_FCompare(IMX(i, k), IMX(i - 1, k) + TSC(p7P_II, k) + ISC(k), tol) == eslOK) 
               scur = p7T_I;
         ...
         default: 
            ESL_EXCEPTION(eslFAIL, "bogus state in traceback");
      ...
      // if (scur > 0 && scur < 4) {
      //    printf("%s:\t(%d,%d)  \t%f\n", states[scur], i, k, dp[(i)][(k) * p7G_NSCELLS + state_num[scur] ] );
      // } 
      // else if (scur == p7T_E || scur == p7T_N || scur == p7T_J || scur == p7T_C || scur == p7T_B) {
      //    printf("%s:\t(%d,%d)  \t%f\n", states[scur], i, k, xmx[(i) * p7G_NXCELLS + (state_num[scur])] );
      // }
      // else {
      //    printf("%s:\t(%d,%d)  \n", states[scur], i, k);
      // }
