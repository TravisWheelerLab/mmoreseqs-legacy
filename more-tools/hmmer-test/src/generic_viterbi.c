...
p7_GViterbi(const ESL_DSQ *dsq, int L, const P7_PROFILE *gm, P7_GMX *gx, float *opt_sc)
{
   ...
   for (i = 1; i <= L; i++)
   {
      /* get row according the ith amino in query sequence */
      float const *rsc = gm->rsc[dsq[i]];
      float sc;

      /* Initialization of the zero column. */
      ...
      {
         /* match state */
         sc1       = MMX(i - 1, k - 1)   + TSC(p7P_MM, k - 1);
         sc2       = IMX(i - 1, k - 1)   + TSC(p7P_IM, k - 1);
         sc3       = DMX(i - 1, k - 1)   + TSC(p7P_DM, k - 1);
         sc4       = XMX(i - 1, p7G_B)   + TSC(p7P_BM, k - 1);
         sc        = ESL_MAX( ESL_MAX(sc1, sc2), ESL_MAX(sc3, sc4) );
         MMX(i, k) = sc + MSC(k);

         // DAVID RICH EDIT
         // printf("(%d, %d)  \tMM: %.3f\t IM: %.3f\t DM: %.3f\t BM: %.3f\t MSC: %.3f\n", i, k, sc1, sc2, sc3, sc4, MSC(k));
         ...
         sc1 = XMX(i, p7G_E);
         sc2 = MMX(i, k) + esc;
         sc = XMX(i, p7G_E) = ESL_MAX(XMX(i, p7G_E), MMX(i, k) + esc);
