...
esl_sq_Compare(ESL_SQ *sq1, ESL_SQ *sq2)
{
   ...
   } else if (sq1->ss != NULL || sq2->ss != NULL)  return eslFAIL;
   ...
   for (x = 0; x < sq1->nxr; x++) {
      ...
      } else if (sq1->xr_tag[x] != NULL || sq2->xr_tag[x] != NULL) return eslFAIL;

      if (sq1->xr[x] != NULL && sq2->xr[x] != NULL) {
         if (strcmp(sq1->xr[x], sq2->xr[x]) != 0)      return eslFAIL;
      } else if (sq1->xr[x] != NULL || sq2->xr[x] != NULL) return eslFAIL;
   }
