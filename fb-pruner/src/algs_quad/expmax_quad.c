                        ...
                        float*               sc_final )
{
   ...
   /* Forward */
   run_ExpMax_Forward_Quad( query, target, Q, T, st_MX_fwd, sp_MX_fwd, sc_final );
   /* Backward */
   run_ExpMax_Backward_Quad( query, target, Q, T, st_MX_bck, sp_MX_bck, sc_final );
   /* multiply results of Forward and Backward */
   for (int k = 0; k <= NUM_NORMAL_STATES; k++) {
      for (int i = 0; i <= Q; i++) {
         for (int j = 0; j <= T; j++) {
            double fwd_sc = MX_3D( st_MX_fwd, k, i, j );
            double bck_sc = MX_3D( st_MX_bck, k, i, j );
            MX_3D( st_MX_fwd, k, i, j ) = logsum( fwd_sc, bck_sc );
         }
      }
   }
   /* Viterbi */
...
/* FUNCTION: run_ExpMax_Bck_Quad()
 * SYNOPSIS: Perform Backward step of Expectation Maximization.
 *
 * PURPOSE:
 *
 *  ARGS:      <query>     query sequence, 
...
/*
 *  FUNCTION:  run_ExpMax_Viterbi_Quad()
 *  SYNOPSIS:  Run Viterbi step of Expectation Maximization
 *
 *  PURPOSE:
 *
 *  ARGS:      <query>     query sequence,
