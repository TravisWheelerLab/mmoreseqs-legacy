                     ...
                     DOMAIN_DEF*    dom_def )         /* OUTPUT: domain data */
{
   ...
   //       q_seq, t_prof, &Q_range, &T_range, st_MX_fwd, sp_MX_fwd, &sc );
   fprintf(stdout, "# score: %7.3f\n", sc);
   fprintf(stdout, "# ==> Ranged Backward\n");
   run_Forward_Quad(
      q_seq, t_prof, Q, T, st_MX_fwd, sp_MX_fwd, &sc );
   // run_Ranged_Backward_Quad( 
   //       q_seq, t_prof, &Q_range, &T_range, st_MX_bck, sp_MX_bck, &sc );
   fprintf(stdout, "# score: %7.3f\n", sc);
   ...
   run_Null2_ByExpectation( q_seq, t_prof, Q, T, &Q_range, &T_range, 
                           ...
                           DOMAIN_DEF*       dom_def )         /* OUTPUT: domain def's null2_sc vector */
{
   ...
   VECTOR_FLT_GrowTo( dom_def->st_freq, T * NUM_NORMAL_STATES );
