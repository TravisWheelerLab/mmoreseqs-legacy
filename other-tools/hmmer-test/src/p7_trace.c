...
int p7_trace_Grow(P7_TRACE *tr)
{
  ...
  if (tr->pp != NULL)
    ESL_RALLOC(tr->pp, tmp, sizeof(float) * 2 * tr->nalloc);
  tr->nalloc *= 2;
  return eslOK;
...
int p7_trace_GrowTo(P7_TRACE *tr, int N)
{
  ...
  if (tr->pp != NULL)
    ESL_RALLOC(tr->pp, tmp, sizeof(float) * N);
  tr->nalloc = N;
  return eslOK;
...
int p7_trace_GetStateUseCounts(const P7_TRACE *tr, int *counts)
{
  ...
  for (x = 0; x < p7T_NSTATETYPES; x++)
    counts[x] = 0;

  for (z = 0; z < tr->N; z++)
  {
    x = tr->st[z];
    if (x < 0 || x >= p7T_NSTATETYPES)
      ESL_EXCEPTION(eslEINVAL, "bad state type");
    counts[x]++;
  }
  return eslOK;
}
 ...
 */
int p7_trace_GetDomainCoords(const P7_TRACE *tr, int which,
                             int *ret_i1, int *ret_i2, int *ret_k1, int *ret_k2)
{
  ...
  if (which < 0)
    ESL_XEXCEPTION(eslEINVAL, "bad which < 0");
   ...
   */
  if (tr->st[z] != p7T_M)
    ESL_XEXCEPTION(eslECORRUPT, "not a profile trace?");
  *ret_i1 = tr->i[z];
  *ret_k1 = tr->k[z];
  ...
  for (; z < tr->N; z++)
    if (tr->st[z] == p7T_E)
      break;
  if (z == tr->N)
    ESL_EXCEPTION(eslECORRUPT, "invalid trace: no E for a B");
  do
  {
    z--;
  } while (tr->st[z] == p7T_D); /* roll back over any D trailer */
  if (tr->st[z] != p7T_M)
    ESL_EXCEPTION(eslECORRUPT, "invalid trace: no M");
  *ret_i2 = tr->i[z];
  *ret_k2 = tr->k[z];
  return eslOK;
...
int p7_trace_Validate(const P7_TRACE *tr, const ESL_ALPHABET *abc, const ESL_DSQ *dsq, char *errbuf)
{
   ...
   */
  if (tr->N < 3)
    ESL_FAIL(eslFAIL, errbuf, "trace is too short");
  if (tr->N > tr->nalloc)
    ESL_FAIL(eslFAIL, errbuf, "N of %d isn't sensible", tr->N);
  ...
  if (tr->st[0] == p7T_B)
    is_core = TRUE;
  else if (tr->st[0] == p7T_S)
    is_core = FALSE;
  else
    ESL_FAIL(eslFAIL, errbuf, "first state neither S nor B");
   ...
   */
  if (is_core && tr->st[tr->N - 1] != p7T_E)
    ESL_FAIL(eslFAIL, errbuf, "last state not E");
  if (!is_core && tr->st[tr->N - 1] != p7T_T)
    ESL_FAIL(eslFAIL, errbuf, "last state not T");
  if (tr->k[0] != 0)
    ESL_FAIL(eslFAIL, errbuf, "first state shouldn't have k set");
  if (tr->i[0] != 0)
    ESL_FAIL(eslFAIL, errbuf, "first state shouldn't have i set");
  if (tr->k[tr->N - 1] != 0)
    ESL_FAIL(eslFAIL, errbuf, "last state shouldn't have k set");
  if (tr->i[tr->N - 1] != 0)
    ESL_FAIL(eslFAIL, errbuf, "last state shouldn't have i set");

  if (tr->pp != NULL && tr->pp[0] != 0.0)
    ESL_FAIL(eslFAIL, errbuf, "first state doesn't emit; but post prob isn't 0");
  if (tr->pp != NULL && tr->pp[tr->N - 1] != 0.0)
    ESL_FAIL(eslFAIL, errbuf, "last state doesn't emit; but post prob isn't 0");
  ...
  for (z = 1; z < tr->N - 1; z++)
  {
    for (; dsq[i] != eslDSQ_SENTINEL; i++) /* find next non-gap residue in dsq */
      if (esl_abc_XIsResidue(abc, dsq[i]) || esl_abc_XIsNonresidue(abc, dsq[i]))
        break; /* '*' included as emitted "residue"  */
    ...
    case p7T_X:
      if (!is_core)
        ESL_FAIL(eslFAIL, errbuf, "X state (missing data) only appears in core traces");
    ...
    case p7T_N:
      if (is_core)
        ESL_FAIL(eslFAIL, errbuf, "core trace can't contain N");
      if (tr->k[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "no N should have k set");
      if (prv == p7T_S)
      { /* 1st N doesn't emit */
        if (tr->i[z] != 0)
          ESL_FAIL(eslFAIL, errbuf, "first N shouldn't have i set");
        if (tr->pp != NULL && tr->pp[z] != 0.0)
          ESL_FAIL(eslFAIL, errbuf, "first N can't have nonzero post prob");
      }
      else if (prv == p7T_N)
      { /* subsequent N's do */
        if (tr->i[z] != i)
          ESL_FAIL(eslFAIL, errbuf, "expected i doesn't match trace's i");
        i++;
      }
      else
        ESL_FAIL(eslFAIL, errbuf, "bad transition to N; expected {S,N}->N");
      break;

    case p7T_B:
      if (tr->k[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "B shouldn't have k set");
      if (tr->i[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "B shouldn't have i set");
      if (tr->pp != NULL && tr->pp[z] != 0.0)
        ESL_FAIL(eslFAIL, errbuf, "B can't have nonzero post prob");
    ...
    case p7T_M:
      if (prv == p7T_B)
        ...
        k++; /* on a B->Mk entry, trust k; else verify */

      if (tr->k[z] != k)
        ESL_FAIL(eslFAIL, errbuf, "expected k doesn't match trace's k");
      if (tr->i[z] != i)
        ESL_FAIL(eslFAIL, errbuf, "expected i doesn't match trace's i");
    ...
    case p7T_D:
      k++;
      if (tr->st[z - 1] == p7T_X)
        k = tr->k[z]; /* with fragments, a X->Ik case is possible */
      if (tr->k[z] != k)
        ESL_FAIL(eslFAIL, errbuf, "expected k doesn't match trace's k");
      if (tr->i[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "D shouldn't have i set");
      if (tr->pp != NULL && tr->pp[z] != 0.0)
        ESL_FAIL(eslFAIL, errbuf, "D can't have nonzero post prob");
      if (is_core)
      {
    ...
    case p7T_E:
      if (tr->k[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "E shouldn't have k set");
      if (tr->i[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "E shouldn't have i set");
      if (tr->pp != NULL && tr->pp[z] != 0.0)
        ESL_FAIL(eslFAIL, errbuf, "E can't have nonzero post prob");
      if (is_core)
      {
    ...
    case p7T_J:
      if (tr->k[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "no J should have k set");
      if (prv == p7T_E)
      { /* 1st J doesn't emit */
        if (tr->i[z] != 0)
          ESL_FAIL(eslFAIL, errbuf, "first J shouldn't have i set");
        if (tr->pp != NULL && tr->pp[z] != 0.0)
          ESL_FAIL(eslFAIL, errbuf, "first J can't have nonzero post prob");
      }
      else if (prv == p7T_J)
      { /* subsequent J's do */
        if (tr->i[z] != i)
        ...
        ESL_FAIL(eslFAIL, errbuf, "bad transition to J; expected {E,J}->J");
      break;

    case p7T_C:
      if (is_core)
        ESL_FAIL(eslFAIL, errbuf, "core trace can't contain C");
      if (tr->k[z] != 0)
        ESL_FAIL(eslFAIL, errbuf, "no C should have k set");
      if (prv == p7T_E)
      { /* 1st C doesn't emit */
        if (tr->i[z] != 0)
          ESL_FAIL(eslFAIL, errbuf, "first C shouldn't have i set");
        if (tr->pp != NULL && tr->pp[z] != 0.0)
          ESL_FAIL(eslFAIL, errbuf, "first C can't have nonzero post prob");
      }
      else if (prv == p7T_C)
      { /* subsequent C's do */
        if (tr->i[z] != i)
        ...
        ESL_FAIL(eslFAIL, errbuf, "bad transition to C; expected {E,C}->C");
      break;
  ...
  if (k > tr->M)
    ESL_FAIL(eslFAIL, errbuf, "M=%d, but k went to %d\n", tr->M, k);
  if (i - 1 > tr->L)
    ESL_FAIL(eslFAIL, errbuf, "L=%d, but i went to %d\n", tr->L, i);
 ...
 *            output.
 */
int p7_trace_Dump(FILE *fp, const P7_TRACE *tr, const P7_PROFILE *gm, const ESL_DSQ *dsq) /* replace void w/ P7_PROFILE */
{
...
int p7_trace_Compare(P7_TRACE *tr1, P7_TRACE *tr2, float pptol)
{
  ...
  if (tr1->pp != NULL && tr2->pp != NULL)
  {
    for (z = 0; z < tr1->N; z++)
      ...
      {
        if (esl_FCompare(tr1->pp[z], tr2->pp[z], pptol) != eslOK)
          esl_fatal("FAIL");
      }
      else
      {
        if (tr1->pp[z] != tr2->pp[z])
          esl_fatal("FAIL"); /* both 0.0 */
      }
  }
...
int p7_trace_SetPP(P7_TRACE *tr, const P7_GMX *pp)
{
  ...
  if (tr->pp == NULL)
    ESL_ALLOC(tr->pp, sizeof(float) * tr->nalloc);
  ...
  for (z = 0; z < tr->N; z++)
  {
    if (tr->i[z] > 0) /* an emitting state? */
    {
      ...
      {
      case p7T_M:
        tr->pp[z] = MMX(tr->i[z], tr->k[z]);
        break;
      case p7T_I:
        tr->pp[z] = IMX(tr->i[z], tr->k[z]);
        break;
      case p7T_N:
        tr->pp[z] = XMX(tr->i[z], p7G_N);
        break;
      case p7T_C:
        tr->pp[z] = XMX(tr->i[z], p7G_C);
        break;
      case p7T_J:
        tr->pp[z] = XMX(tr->i[z], p7G_J);
        break;
      default:
        ESL_EXCEPTION(eslEINVAL, "no such emitting state");
      }
    }
    else
      tr->pp[z] = 0.0;
...
int p7_trace_Append(P7_TRACE *tr, char st, int k, int i)
{
  ...
  if ((status = p7_trace_Grow(tr)) != eslOK)
    return status;
...
int p7_trace_Reverse(P7_TRACE *tr)
{
  ...
  {
    tmp = tr->st[tr->N - z - 1];
    tr->st[tr->N - z - 1] = tr->st[z];
    tr->st[z] = tmp;
    tmp = tr->k[tr->N - z - 1];
    tr->k[tr->N - z - 1] = tr->k[z];
    tr->k[z] = tmp;
    tmp = tr->i[tr->N - z - 1];
    tr->i[tr->N - z - 1] = tr->i[z];
    tr->i[z] = tmp;
    if (tr->pp != NULL)
    {
      tmpf = tr->pp[tr->N - z - 1];
      tr->pp[tr->N - z - 1] = tr->pp[z];
      tr->pp[z] = tmpf;
    }
  }
...
int p7_trace_Index(P7_TRACE *tr)
{
  ...
  for (z = 0; z < tr->N; z++)
  {
    ...
    {
    case p7T_B:
      if ((status = p7_trace_GrowIndex(tr)) != eslOK)
        goto ERROR;
      tr->tfrom[tr->ndom] = z;
      tr->sqfrom[tr->ndom] = 0;
    ...
    case p7T_M:
      if (tr->sqfrom[tr->ndom] == 0)
        tr->sqfrom[tr->ndom] = tr->i[z];
      if (tr->hmmfrom[tr->ndom] == 0)
...
int p7_trace_FauxFromMSA(ESL_MSA *msa, int *matassign, int optflags, P7_TRACE **tr)
{
  ...
  for (idx = 0; idx < msa->nseq; idx++)
  {
    if ((tr[idx] = p7_trace_Create()) == NULL)
      goto ERROR;
    if ((status = p7_trace_Append(tr[idx], p7T_B, 0, 0)) != eslOK)
      goto ERROR;
    ...
    if ((status = p7_trace_Append(tr[idx], p7T_E, 0, 0)) != eslOK)
      goto ERROR;
...
int p7_trace_Count(P7_HMM *hmm, ESL_DSQ *dsq, float wt, P7_TRACE *tr)
{
  ...
  for (z = z1; z < z2; z++)
  {
    ...
    if (st == p7T_B)
    {
      ...
      else
      {
        ...
        case p7T_D:
          ...
          break;
        default:
          ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
        }
      }
    }
    else if (st == p7T_M)
    {
      ...
      case p7T_E:
        hmm->t[k][p7H_MM] += wt;
        break; /* k==M. A local alignment would've been Mk->X->E. */
      default:
        ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
      }
    }
    ...
    else if (st == p7T_D)
    {
      ...
      case p7T_E:
        hmm->t[k][p7H_DM] += wt;
        break; /* k==M. A local alignment would've been Dk->X->E. */
      default:
        ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
      }
    }
...
utest_faux(ESL_MSA *msa, int *matassign, int M)
{
  ...
  for (i = 0; i < msa->nseq; i++)
    if (esl_sq_FetchFromMSA(msa, i, &(sq[i])) != eslOK)
      esl_fatal(msg);

  if (p7_trace_FauxFromMSA(msa, matassign, p7_MSA_COORDS, tr) != eslOK)
    esl_fatal(msg);
  if (p7_tracealign_MSA(msa, tr, M, optflags, &msa2) != eslOK)
    esl_fatal(msg);
  if (esl_msa_Compare(msa, msa2) != eslOK)
    esl_fatal(msg);
  esl_msa_Destroy(msa2);
  for (i = 0; i < msa->nseq; i++)
    p7_trace_Destroy(tr[i]);

  if (p7_trace_FauxFromMSA(msa, matassign, p7_DEFAULT, tr) != eslOK)
    esl_fatal(msg);
  if (p7_tracealign_Seqs(sq, tr, msa->nseq, M, optflags, NULL, &msa2) != eslOK)
    esl_fatal(msg);
  if (esl_msa_Compare(msa, msa2) != eslOK)
    esl_fatal(msg);
...
int main(int argc, char **argv)
{
  ...
  if ((msa = esl_msa_CreateFromString("# STOCKHOLM 1.0\n#=GC RF .xxxx.\nseq1    AAAAAA\nseq2    -AAA--\nseq3    AA--AA\n//\n", eslMSAFILE_STOCKHOLM)) == NULL)
    esl_fatal(msg);
  if (esl_msa_Digitize(abc, msa, NULL) != eslOK)
    esl_fatal(msg);
