...
int p7_OptimalAccuracy(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e)
{
  ...
  ox->L = pp->L;
  for (q = 0; q < Q; q++)
    MMO(dpc, q) = IMO(dpc, q) = DMO(dpc, q) = infv;
  ...
  for (i = 1; i <= pp->L; i++)
  {
    ...
    for (q = 0; q < Q; q++)
    {
      sv = _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), xBv);
      tp++;
      sv = _mm_max_ps(sv, _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), mpv));
      tp++;
      sv = _mm_max_ps(sv, _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), ipv));
      tp++;
      sv = _mm_max_ps(sv, _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), dpv));
      tp++;
      sv = _mm_add_ps(sv, *ppp);
      ppp += 2;
      ...
      dcv = _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), sv);
      tp++;

      sv = _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), mpv);
      tp++;
      sv = _mm_max_ps(sv, _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), ipv));
      tp++;
      IMO(dpc, q) = _mm_add_ps(sv, *ppp);
      ppp++;
    }
    ...
    for (q = 0; q < Q; q++)
    {
      ...
      dcv = _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), DMO(dpc, q));
      tp++;
    }
    ...
    for (j = 1; j < 4; j++)
    {
      ...
      {
        DMO(dpc, q) = _mm_max_ps(dcv, DMO(dpc, q));
        dcv = _mm_and_ps(_mm_cmpgt_ps(*tp, zerov), dcv);
        tp++;
      }
    }
...
{
  printf("=== p7_OATrace() [BEGIN] ===\n");
  FILE* fp = NULL;
  fp = fopen("test_output/hmmer.optimal_traceback.opt.tsv", "w+");
  ...
  if (tr->N != 0)
    ESL_EXCEPTION(eslEINVAL, "trace not empty; needs to be Reuse()'d?");

  if ((status = p7_trace_AppendWithPP(tr, p7T_T, k, i, 0.0)) != eslOK)
    return status;
  if ((status = p7_trace_AppendWithPP(tr, p7T_C, k, i, 0.0)) != eslOK)
    return status;
  ...
  while (s0 != p7T_S)
  {
    ...
    {
    case p7T_M:
      s1 = select_m(om, ox, i, k);
      k--;
      i--;
      break;
    case p7T_D:
      s1 = select_d(om, ox, i, k);
      k--;
      break;
    case p7T_I:
      s1 = select_i(om, ox, i, k);
      i--;
      break;
    case p7T_N:
      s1 = select_n(i);
      break;
    case p7T_C:
      s1 = select_c(om, pp, ox, i);
      break;
    case p7T_J:
      s1 = select_j(om, pp, ox, i);
      break;
    case p7T_E:
      s1 = select_e(om, ox, i, &k);
      break;
    case p7T_B:
      s1 = select_b(om, ox, i);
      break;
    ...
    if (s1 == -1) {
      ESL_EXCEPTION(eslEINVAL, "OA traceback choice failed");
    }
      
    postprob = get_postprob(pp, s1, s0, k, i);
    fprintf(fp, "[%d] { %s, (%d,%d) }: %11.4f %11.4f\n", tr->N, STATE_NAMES[s1], k, i, postprob, log(postprob) );

    if ((status = p7_trace_AppendWithPP(tr, s1, k, i, postprob)) != eslOK)
      return status;

    if ((s1 == p7T_N || s1 == p7T_J || s1 == p7T_C) && s1 == s0)
      i--;
    s0 = s1;
...
get_postprob(const P7_OMX *pp, int scur, int sprv, int k, int i)
{
  ...
  case p7T_N:
    if (sprv == scur)
      return pp->xmx[i * p7X_NXCELLS + p7X_N];
  case p7T_C:
    if (sprv == scur)
      return pp->xmx[i * p7X_NXCELLS + p7X_C];
  case p7T_J:
    if (sprv == scur)
      return pp->xmx[i * p7X_NXCELLS + p7X_J];
  default:
    return 0.0;
  }
}
...
select_m(const P7_OPROFILE *om, const P7_OMX *ox, int i, int k)
{
  ...
  u.v = xBv;
  tv.v = *tp;
  path[3] = ((tv.p[r] == 0.0) ? -eslINFINITY : u.p[r]);
  tp++;
  u.v = mpv;
  tv.v = *tp;
  path[0] = ((tv.p[r] == 0.0) ? -eslINFINITY : u.p[r]);
  tp++;
  u.v = ipv;
  tv.v = *tp;
  path[1] = ((tv.p[r] == 0.0) ? -eslINFINITY : u.p[r]);
  tp++;
  u.v = dpv;
  tv.v = *tp;
  path[2] = ((tv.p[r] == 0.0) ? -eslINFINITY : u.p[r]);
...
select_i(const P7_OPROFILE *om, const P7_OMX *ox, int i, int k)
{
  ...
  mpv.v = ox->dpf[i - 1][q * 3 + p7X_M];
  tv.v = *tp;
  path[0] = ((tv.p[r] == 0.0) ? -eslINFINITY : mpv.p[r]);
  tp++;
  ipv.v = ox->dpf[i - 1][q * 3 + p7X_I];
  tv.v = *tp;
  path[1] = ((tv.p[r] == 0.0) ? -eslINFINITY : ipv.p[r]);
...
utest_optacc(ESL_GETOPTS *go, ESL_RANDOMNESS *r, ESL_ALPHABET *abc, P7_BG *bg, int M, int L, int N)
{
  ...
  while (N--)
  {
    if (p7_ProfileEmit(r, hmm, gm, bg, sq, tro) != eslOK)
      esl_fatal(msg);

    if (p7_omx_GrowTo(ox1, M, sq->n, sq->n) != eslOK)
      esl_fatal(msg);
    if (p7_omx_GrowTo(ox2, M, sq->n, sq->n) != eslOK)
      esl_fatal(msg);
    if (p7_gmx_GrowTo(gx1, M, sq->n) != eslOK)
      esl_fatal(msg);
    if (p7_gmx_GrowTo(gx2, M, sq->n) != eslOK)
      esl_fatal(msg);

    if (p7_Forward(sq->dsq, sq->n, om, ox1, &fsc) != eslOK)
      esl_fatal(msg);
    if (p7_Backward(sq->dsq, sq->n, om, ox1, ox2, &bsc) != eslOK)
      esl_fatal(msg);
    if (p7_Decoding(om, ox1, ox2, ox2) != eslOK)
      esl_fatal(msg);
    if (p7_OptimalAccuracy(om, ox2, ox1, &accscore) != eslOK)
      esl_fatal(msg);
      ...
      p7_gmx_Dump(stdout, gx1, p7_DEFAULT);
#endif
    if (p7_OATrace(om, ox2, ox1, tr) != eslOK)
      esl_fatal(msg);

    if (p7_GForward(sq->dsq, sq->n, gm, gx1, &fsc_g) != eslOK)
      esl_fatal(msg);
    if (p7_GBackward(sq->dsq, sq->n, gm, gx2, &bsc_g) != eslOK)
      esl_fatal(msg);
    ...
    if (p7_GDecoding(gm, gx1, gx2, gx2) != eslOK)
      esl_fatal(msg);
    if (p7_GOptimalAccuracy(gm, gx2, gx1, &accscore_g) != eslOK)
      esl_fatal(msg);
...
#endif
    if (p7_GOATrace(gm, gx2, gx1, trg) != eslOK)
      esl_fatal(msg);

    if (p7_trace_SetPP(tro, gx2) != eslOK)
      esl_fatal(msg);
    ...
    if (p7_trace_Validate(tr, abc, sq->dsq, NULL) != eslOK)
      esl_fatal(msg);
    if (p7_trace_Validate(trg, abc, sq->dsq, NULL) != eslOK)
      esl_fatal(msg);
    if (p7_trace_Compare(tr, trg, pptol) != eslOK)
      esl_fatal(msg);
    ...
    if (esl_FCompare(fsc, bsc, sctol) != eslOK)
      esl_fatal(msg);
    if (esl_FCompare(fsc_g, bsc_g, gtol) != eslOK)
      esl_fatal(msg);
    if (esl_FCompare(fsc, fsc_g, gtol) != eslOK)
      esl_fatal(msg);
    if (esl_FCompare(accscore, accscore_g, gtol) != eslOK)
      esl_fatal(msg);
    if (esl_FCompare(accscore_g, accscore_g2, gtol) != eslOK)
      esl_fatal(msg);
    if (accscore_g2 < accscore_o)
      esl_fatal(msg);
...
int main(int argc, char **argv)
{
  ...
  {
    p7_omx_FDeconvert(ox2, gx);
    p7_gmx_Dump(stdout, gx, p7_DEFAULT);
  }
  if (esl_opt_GetBoolean(go, "-m"))
  {
    p7_omx_FDeconvert(ox1, gx);
    p7_gmx_Dump(stdout, gx, p7_DEFAULT);
  }
