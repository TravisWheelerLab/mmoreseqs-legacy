...
{
  printf("=== p7_GOptimalAccuracy() [BEGIN] ===\n");
...
{
  printf("=== p7_GOATrace() [BEGIN] ===\n");
  FILE* fp = NULL;
  fp = fopen("test_output/hmmer.optimal_traceback.gen.tsv", "w+");
...
#if eslDEBUGLEVEL > 0
  if (tr->N != 0)
    ESL_EXCEPTION(eslEINVAL, "trace isn't empty: forgot to Reuse()?");
#endif
  ...
  while (sprv != p7T_S)
  {
    ...
    {
    case p7T_M:
      scur = select_m(gm, gx, i, k);
      k--;
      i--;
      break;
    case p7T_D:
      scur = select_d(gm, gx, i, k);
      k--;
      break;
    case p7T_I:
      scur = select_i(gm, gx, i, k);
      i--;
      break;
    case p7T_N:
      scur = select_n(i);
    ...
    case p7T_C:
      ...
      break;
    case p7T_J:
      scur = select_j(gm, pp, gx, i);
      break;
    case p7T_E:
      scur = select_e(gm, gx, i, &k);
      break;
    case p7T_B:
      scur = select_b(gm, gx, i);
      break;
    default:
    ...
    fprintf(fp, "[%d] { %s, (%d,%d) }: %11.4f %11.4f\n", tr->N, STATE_NAMES[scur], k, i, postprob, log(postprob) );

    if ((status = p7_trace_AppendWithPP(tr, scur, k, i, postprob)) != eslOK) {
      return status;
    }
...
int main(int argc, char **argv)
{
  ...
  if (esl_opt_GetBoolean(go, "-d"))
    p7_gmx_Dump(stdout, gx2, p7_DEFAULT);
