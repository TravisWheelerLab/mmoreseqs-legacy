...
{
   printf("=> p7_bg_FilterScore, L = %d, M = %d\n", L, bg->fhmm->M );
   ...
   for (int i = 0; i < 2; i++) {
      printf("[ ");
      for (int j = 0; j < bg->abc->K; j++) {
         printf("%7.4f ", bg->fhmm->e[i][j] );
      }
      printf(" ]\n");
   }

   /* create digitaized sequence from character sequence */
