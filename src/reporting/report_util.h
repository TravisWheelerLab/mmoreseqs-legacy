 ...
 */
STATUS_FLAG
REPORT_hr_size(FILE* fp, const int L);

/*!  FUNCTION:    STR_hr_size()
 *   SYNOPSIS:    Create a horizonral rule of specified length.
 */
STATUS_FLAG
STRING_hr_size(char* str_buf, const int L);

/*!  FUNCTION:    REPORT_header()
 *   SYNOPSIS:    Prints field headers with
 */
STATUS_FLAG
REPORT_header(FILE* fp, const char* headers[], const int num_fields);

/*!  FUNCTION:    REPORT_entry()
 *   SYNOPSIS:    Prints tab-delimited of array <data> with length <L> onto line
 * according to its <type>. If data is type float or double, <sig_digits>
 ...
 *   SYNOPSIS:    Prints array of <data>, one entry per line, with <header>
 * labels.
 */
STATUS_FLAG
 ...
 */
STATUS_FLAG
REPORT_data(FILE* fp, const GEN data, const int sig_digits);
