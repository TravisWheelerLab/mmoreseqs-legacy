 ...
 */
void REPORT_mythreshout_header(WORKER* worker, FILE* fp);

/*!   FUNCTION:   REPORT_mythreshout_entry()
 */
void REPORT_mythreshout_entry(WORKER* worker, RESULT* result, FILE* fp);

/*!   FUNCTION:   REPORT_mythreshout_footer()
 *    SYNOPSIS:   Print footer
 *                (modeled after HMMER, see example)
 */
void REPORT_mythreshout_footer(WORKER* worker, FILE* fp);
