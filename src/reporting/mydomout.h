 ...
 */
void REPORT_mydomout_header(WORKER* worker, FILE* fp);

/*!   FUNCTION:   REPORT_myout_entry()
 *    SYNOPSIS:   Print all alignment data for current search
 *                (modeled after HMMER, see example)
 */
void REPORT_mydomout_entry(WORKER* worker, RESULT* result, FILE* fp);

/*!   FUNCTION:   REPORT_myout_footer()
 *    SYNOPSIS:   Print footer
 *                (modeled after HMMER, see example)
 */
void REPORT_mydomout_footer(WORKER* worker, FILE* fp);
