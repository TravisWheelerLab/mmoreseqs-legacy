/*******************************************************************************
 *  - FILE:  testing.h
 *  - DESC:  Testing for navigating through the matrices.
 *******************************************************************************/
 ...
 */
void TEST_set_color(char* color, bool bold);

/*! FUNCTION:  TEST_set_color_num()
 *  SYNOPSIS:  Set console text color by index number <color> and boolean
 * <bold>.
 */
void TEST_set_color_num(int color, bool bold);

/*! FUNCTION:  TEST_fwd_cycle()
 *  SYNOPSIS:  Cycle through all indices in quadratic matrix in forward
 * direction, antidiag-by-antidiag
 */
 ...
 *  SYNOPSIS:  Cycle through all indices in quadratic matrix in backward
 * direction, antidiag-by-antidiag
 */
 ...
 *  SYNOPSIS:  Cycle through all indices in linear matrix in backward direction,
 * antidiag-by-antidiag
 */
 ...
 *  SYNOPSIS:  Fill MATRIX_2D with value according to EDGEBOUNDS, returns number
 * of cells covered by EDGEBOUNDS
 */
int MATRIX_2D_Cloud_Fill(MATRIX_2D* cloud_MX, EDGEBOUNDS* edg, float val);

/*! FUNCTION:  MATRIX_2D_Cloud_Compare()
 *  SYNOPSIS:  Compare <cloud_MX_a> and <cloud_MX_b> ( equality is if cells are
 * both positive, negative, or zero ). When debugging, stores heatmap of
 * differences into <debugger->cloud_MX>.
 */
int MATRIX_2D_Cloud_Compare(MATRIX_2D* cloud_MX_a, MATRIX_2D* cloud_MX_b);
 ...
 */
int MATRIX_2D_CountVal(MATRIX_2D* cloud_MX, float val);

/*! FUNCTION:  MATRIX_2D_Total()
 *  SYNOPSIS:  Sum all cell values in matrix.
 */
float MATRIX_2D_Total(MATRIX_2D* cloud_MX);
