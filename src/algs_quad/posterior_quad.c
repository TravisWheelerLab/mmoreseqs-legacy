...
int run_Posterior_Quad(SEQUENCE* q_seq,       /* query sequence */
...
/* METHOD 1: For Testing Only. Use full forward/backward, for checking Posterior computation */
#if POST_METHOD == 0 || POST_METHOD == 1
  {
...
#if DEBUG
    {
      ...
      DP_MATRIX_Save(Q_size, T_size, st_MX_fwd, sp_MX_fwd, DEBUG_FOLDER "/my.full_fwd.000.mx");
      DP_MATRIX_Save(Q_size, T_size, st_MX_bck, sp_MX_bck, DEBUG_FOLDER "/my.full_bck.000.mx");
    }
#endif
...
#if DEBUG
    {
      fp = fopen(DEBUG_FOLDER "/my.full_post.000.mx", "w+");
...
/* ISSUE: This does not work because it only computes special states on constrained range */
#if POST_METHOD == 0 || POST_METHOD == 2
  {
...
#if DEBUG
    {
      ...
      DP_MATRIX_Save(Q_size, T_size, st_MX_fwd, sp_MX_fwd, DEBUG_FOLDER "/my.constrained_fwd.000.mx");
      DP_MATRIX_Save(Q_size, T_size, st_MX_bck, sp_MX_bck, DEBUG_FOLDER "/my.constrained_bck.000.mx");
    }
#endif
...
#if DEBUG
    {
      fp = fopen(DEBUG_FOLDER "/my.constrained_post.000.mx", "w+");
      DP_MATRIX_Log_Dump(Q_size, T_size, st_MX_post, sp_MX_post, fp);
      fclose(fp);
    }
#endif
...
/* METHOD 3: Use ranged forward/backward, which only computes for given range */
#if POST_METHOD == 0 || POST_METHOD == 3
  {
...
#if DEBUG
    {
      ...
      DP_MATRIX_Save(Q_size, T_size, st_MX_fwd, sp_MX_fwd, DEBUG_FOLDER "/my.ranged_fwd.000.mx");
      DP_MATRIX_Save(Q_size, T_size, st_MX_bck, sp_MX_bck, DEBUG_FOLDER "/my.ranged_bck.000.mx");
    }
#endif
...
#if DEBUG
    {
      fp = fopen(DEBUG_FOLDER "/my.ranged_post.000.mx", "w+");
      DP_MATRIX_Log_Dump(Q_size, T_size, st_MX_post, sp_MX_post, fp);
      fclose(fp);
    }
#endif
  ...
  null2_seq_bias = dom_def->null2_seq_bias;

  fprintf(stdout, "# null2_seq_bias: %.9f\n", null2_seq_bias);
...
int run_Decode_Posterior_Quad(SEQUENCE* q_seq,       /* query sequence */
                              HMM_PROFILE* t_prof,   /* target hmm model */
...
int run_Decode_Special_Posterior_Quad(SEQUENCE* q_seq,       /* query sequence */
                                      HMM_PROFILE* t_prof,   /* target hmm model */
                                      int Q,                 /* query length */
                                      int T,                 /* target length */
                                      MATRIX_2D* sp_MX_fwd,  /* special state matrix for forward */
...
int run_Null2_ByExpectation_Quad(SEQUENCE* query,       /* query sequence */
                                 ...
                                 DOMAIN_DEF* dom_def)   /* OUTPUT: domain def's null2_sc vector */
{
...
#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.2.quad.000.csv", "w+");
    fprintf(fp, "<2>\n");
...
#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.3.quad.000.csv", "w+");
    fprintf(fp, "<3>\n");
    fprintf(fp, "==> NORMAL STATES (st_freq) <=\n");
    for (int t_0 = T_beg; t_0 < T_end; t_0++) {
      tx0 = t_0 - T_beg;
...
#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.4.quad.000.csv", "w+");
    fprintf(fp, "<4>\n");
    fprintf(fp, "==> NORMAL STATES (st_freq) <=\n");
    for (int t_0 = T_beg; t_0 < T_end; t_0++) {
      tx0 = t_0 - T_beg;
  ...
  {
    // fp = stdout;
    fp = fopen(DEBUG_FOLDER "/my.null2.quad.000.csv", "w+");
  ...
  dom_def->null2_seq_bias = 0.0f;
  for (q_0 = Q_beg; q_0 < Q_end; q_0++) {
    qx0 = q_0 - Q_beg;
    k_0 = AA_REV[query->seq[q_0]];
    dom_def->null2_seq_bias += logf(VEC_X(dom_def->null2_sc, k_0));
  }

#if DEBUG
  {
    // fp = stdout;
    fp = fopen(DEBUG_FOLDER "/my.null2sc.quad.000.csv", "w+");
