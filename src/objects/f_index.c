/*******************************************************************************
 *  - FILE:      f_index.h
 *  - DESC:    F_INDEX Object.  Indexes HMM and FASTA files for fast lookup.
 *******************************************************************************/
                      ...
                      int hi)            /* upper end of range in subarray  */
{
  ...
  while (true) {
    ...
    while (left <= right) {
      if (F_INDEX_Compare_by_Name(&(arr[left]), &(arr[0])) > 0)
        break;
      left++;
    }
    ...
    while (left <= right) {
      if (F_INDEX_Compare_by_Name(&(arr[right]), &(arr[0])) <= 0)
        break;
      right--;
    }
