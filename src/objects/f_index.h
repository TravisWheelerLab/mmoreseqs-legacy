 ...
 */
void F_INDEX_Pushback(F_INDEX* index, F_INDEX_NODE* node);

/*!  FUNCTION:    F_INDEX_Resize()
 *   SYNOPSIS:    Resizes the length of the F_INDEX nodes array.
 */
void F_INDEX_Resize(F_INDEX* index, int size);

/*!  FUNCTION:    F_INDEX_Get()
 *   SYNOPSIS:    Gets the NODE position <idx> in <index>.
 */
F_INDEX_NODE* F_INDEX_Get(F_INDEX* index, int idx);
 ...
 *     RETURN:   Search result; NULL if no result found.
 */
F_INDEX_NODE* F_INDEX_Getby_Id(F_INDEX* index, int search_term);
