/*******************************************************************************
 *  - FILE:      structs.h
 *  - DESC:    Macros used by fb-pruner.  Some set by Makefile.
 *******************************************************************************/
...
// #define BUILD_DESCRIPT     "Heuristic Pruned Forward-Backward for Faster
// Profile-to-Sequence Search"
...
// #endif
// /* location of bash and python scripts */
// #ifndef SCRIPT_LOC
// #define SCRIPT_LOC				MACRO_XSTR(PROJECT_LOC) "/"
// MACRO_XSTR(MMORE_BIN_LOC) #endif
// /* === TOOLS === */
...
#define printf_v(v_threshold, ...)          \
  if (args->verbose_level >= (v_threshold)) \
  printf(__VA_ARGS__)
...
/* print all (including debugging) */
#define printf_vall(...) printf_v(VERBOSE_ALL, __VA_ARGS__)

/* determines whether output is printed based on verbose_level */
#define fprintf_v(v_threshold, ...)       \
  if (args->verbose_level >= v_threshold) \
  fprintf(__VA_ARGS__)
...
// #define ERROR_calloc(...) 			ERRORCHECK_calloc(__VA_ARGS__,
// ERRORCHECK_locate() )
...
#define MX_3D(mx, st, q_0, t_0) \
  ((mx)->data[((st) * ((mx)->C * (mx)->N)) + ((q_0) * ((mx)->N)) + (t_0)])
#endif
...
/* match, insert, delete for st_MX matrix (quadratic space matrix) (specify
 * matrix) */
...
/* match, insert, delete for st_MX3 matrix (linear space matrix) (specify
 * matrix) */
...
#define SMX_X(mx, st, qx0, tx0) \
  (*MATRIX_3D_SPARSE_GetX_byOffset((mx), (qx0), (tx0), (st)))
#endif
#if (MATRIX_FUNCTIONS == FALSE)
/* generic access for MATRIX_3D_SPARSE via direct data access */
#define SMX_X(mx, st, qx0, tx0) \
  (mx->data->data[(qx0) + ((tx0)*NUM_NORMAL_STATES) + (st)])
#endif
...
/* === TRANSITION SCORE, SPECIAL TRANSITION SCORE, MATCH SCORE, INSERT SCORE
 * MACROS === */
...
/* number of float vectors per sequence (striped implementation requires at
 * least two vectors) */
#define NUM_VEC_FOR_SEQ(seq_length, data_per_vec) \
  MATH_Max(2, (seq_length / data_per_vec) + 1)
