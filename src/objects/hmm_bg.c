...
HMM_BG_Destroy(HMM_BG* bg) {
  if (bg != NULL) {
    if (bg->fhmm != NULL)
      esl_alphabet_Destroy(bg->abc);
    bg->f = ERROR_free(bg->f);
    if (bg->fhmm != NULL)
      esl_hmm_Destroy(bg->fhmm);
    if (bg->sq != NULL) {
 ...
 *
 *   - DESC:   Sets the geometric null model length
 ...
 *
 *   - DESC:   Calculate the null1 log score, for sequence <dsq>
 ...
 *
 *   - DESC:  The "filter HMM" is an experimental filter in the
 ...
 *
 *  - DESC:    Modeled after p7_bg_FilterScore() in HMMER.
