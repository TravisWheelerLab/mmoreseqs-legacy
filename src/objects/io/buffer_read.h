 ...
 *   SYNOPSIS:  Split <buffer>'s <line> by <delim> and load into
 * <field_offsets>. RETURN:    Returns number of line splits / fields.
 */
size_t BUFFER_SplitLine(BUFFER* buffer);

/*!  FUNCTION:  BUFFER_SplitLineOn()
 *   SYNOPSIS:  Split <buffer>'s <line> by <delim> and load into
 * <field_offsets>. Uses temporary <delimiter> supplied by user. RETURN: Returns
 * number of line splits / fields.
 */
size_t BUFFER_SplitLineOn(BUFFER* buffer, STR delimiter);
 ...
 */
STR BUFFER_GetField(BUFFER* buffer, size_t i);
 ...
 */
STR BUFFER_NextField(BUFFER* buffer);
