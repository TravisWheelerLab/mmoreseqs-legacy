 ...
 *  RETURN:    Return pointer to new MATRIX_2D object.
 */
int MATRIX_2D_Fill(MATRIX_2D* mx,
                   float val) {
 ...
 *  RETURN:
 */
int MATRIX_2D_Reuse(MATRIX_2D* mx,
                    int R,
                    int C) {
  if (R * C > mx->Nalloc) {
    MATRIX_2D_Resize(mx, R, C);
 ...
 *  RETURN:
 */
int MATRIX_2D_Reuse_Clean(MATRIX_2D* mx,
                          int R,
                          int C) {
  /* TODO: fix this function. Currently does a linear pass of matrix to clean it. */
 ...
 *  RETURN:
 */
int MATRIX_2D_Resize(MATRIX_2D* mx,
                     int R,
                     int C) {
  mx->Nalloc = R * C;
  mx->R = R;
 ...
 *  RETURN:
 */
int MATRIX_2D_Dump(MATRIX_2D* mx,
                   FILE* fp) {
 ...
 *  RETURN:
 */
int MATRIX_2D_Save(MATRIX_2D* mx,
                   char* filename) {
 ...
 *  SYNOPSIS:  Performs logrithmic function logf() on each cell in matrix.
 *
 *  RETURN:    None.
 */
int MATRIX_2D_Logf(MATRIX_2D* mx) {
  for (int i = 0; i < mx->R; i++) {
    for (int j = 0; j < mx->C; j++) {
      if (MX_2D(mx, i, j) > 0.0) {
        MX_2D(mx, i, j) = logf(MX_2D(mx, i, j));
      } else {
        MX_2D(mx, i, j) = -INF;
      }
    }
