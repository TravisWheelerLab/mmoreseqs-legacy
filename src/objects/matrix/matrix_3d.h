 ...
 *  SYNOPSIS:
 */
MATRIX_3D* MATRIX_3D_Create(const int R, const int C, const int N);

/** FUNCTION:  MATRIX_3D_Destroy()
 *  SYNOPSIS:
 */
MATRIX_3D* MATRIX_3D_Destroy(MATRIX_3D* mx);

/** FUNCTION:  MATRIX_3D_Create_Clean()
 *  SYNOPSIS:
 */
MATRIX_3D* MATRIX_3D_Create_Clean(const int R, const int C, const int N);

/** FUNCTION:  MATRIX_3D_Copy()
 *  SYNOPSIS:
 */
MATRIX_3D* MATRIX_3D_Copy(MATRIX_3D* dest, MATRIX_3D* src);

/** FUNCTION:  MATRIX_3D_Fill()
 *  SYNOPSIS:
 */
void MATRIX_3D_Fill(MATRIX_3D* mx, const float val);
 ...
 *  SYNOPSIS:
 */
int MATRIX_3D_Check_Value(MATRIX_3D* mx, const float val);

/** FUNCTION:  MATRIX_3D_Get()
 *  SYNOPSIS:
 */
float* MATRIX_3D_Get(MATRIX_3D* mx, const int i, const int j, const int k);

/** FUNCTION:  MATRIX_3D_GetX()
 *  SYNOPSIS:  Get pointer to cell from <mx> at position <i,j,k>.
 */
float* MATRIX_3D_GetX(MATRIX_3D* mx, const int i, const int j, const int k);

/** FUNCTION:  MATRIX_3D_Get1D()
 *  SYNOPSIS:  Get pointer to cell from <mx> at flat-index position <n>.
 */
float* MATRIX_3D_Get1D(MATRIX_3D* mx, const int n);

/** FUNCTION:  MATRIX_3D_to_1D()
 *  SYNOPSIS:  Converts 3D coords to 1D array access.
 */
int MATRIX_3D_to_1D(const MATRIX_3D* mx, const int i, const int j, const int k);

/** FUNCTION:  MATRIX_3D_Reuse()
 *  SYNOPSIS:  Reuse MATRIX_3D by resizing only if new matrix requires more
 * memory.
 */
float MATRIX_3D_Reuse(MATRIX_3D* mx, const int R, const int C, const int N);

/** FUNCTION:  MATRIX_3D_Reuse_Clean()
 *  SYNOPSIS:  Reuse MATRIX_3D by resizing only if new matrix requires more
 * memory. New memory is set to -INF.
 */
