 ...
 *             For simple non-dynamic types, most of these are pass-through
 *functions that should be optimized out by compiler. Promises the following API
 *for each datatype (XXX is a template datatype):
 ...
 *                -  NOTE: Need to add a size parameter to _ToString(), so
 *buffer doesn't overflows.
 *
 *             Files with "xxx_ext.h" stores functions outside core API (may be
 *moved to objects/ later).
 *
 *   NOTES:
 *    - There is one level of potential dependency: composite types can
 *incorporate primitive types.
 *******************************************************************************/
