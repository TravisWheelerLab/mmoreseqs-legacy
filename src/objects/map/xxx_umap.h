 ...
 */
int XXX_UMAP_Resize(XXX_UMAP* umap, int size);

/*    FUNCTION:   XXX_UMAP_Add()
 *    SYNOPSIS:   Add new {key,value} to {umap}.
 */
int XXX_UMAP_Add(XXX_UMAP* umap, char* key, XXX* value);

/*    FUNCTION:   XXX_UMAP_Get()
 *    SYNOPSIS:   Return {value} associated with {key,value} pair in {umap}.
 */
int XXX_UMAP_Get(XXX_UMAP* umap, char* key, XXX* value);
