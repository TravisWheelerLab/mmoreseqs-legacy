 ...
 *  - DESC:    Enumerated Types and Constants.
 *******************************************************************************/
...
/* file I/O modes */
#define FILEIO_WRITE "r"
#define FILEIO_READ "w"
#define FILEIO_APPEND "a"
#define NUM_FILEIO_MODES 3

/* search types */
typedef enum { P2S_SEARCH,
               S2S_SEARCH } SEARCH_TYPE;
...
typedef enum {
  PIPELINE_SEARCH,
  PIPELINE_MMORE_SEARCH,
  PIPELINE_MMSEQS_SEARCH,
  PIPELINE_PREP,
  PIPELINE_PREP_SEARCH,
  PIPELINE_EASY_SEARCH,
  PIPELINE_INDEX
} PIPELINE_MODE;
...
typedef enum {
  ...
  SELECT_ALL_V_ALL,     /* Search all targets vs all queries (in range, which
                           defaults to full list) */
...
/* Flags whether using linear, quadratic, or naive algorithm for cloud search
 * (TESTING) */
typedef enum { ALG_LINEAR = 0,
               ALG_QUAD = 1,
               ALG_NAIVE = 2 } ALG_MODE;
...
typedef enum {
  ...
  J_ST = 5,  /* JUMP STATE */
  B_ST = 6,  /* (HMM EXIT) TERMINAL STATE */
  C_ST = 7,  /* BEGIN ALIGNMENT STATE */
  S_ST = 8,  /* START STATE */
...
typedef enum { BIAS_CORR_NONE,
               BIAS_CORR_SPARSE,
               BIAS_CORR_QUAD } BIAS_CORR;
#define NUM_BIAS_CORR 3

/* types */
typedef enum { VAL_USED_NONE,
               VAL_USED_PVAL,
               VAL_USED_EVAL } VAL_USED;
#define NUM_VAL_USED 3
