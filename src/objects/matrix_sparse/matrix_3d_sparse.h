 ...
 *  - DESC:    MATRIX_3D_SPARSE Float object.
 *******************************************************************************/
 ...
 */
MATRIX_3D_SPARSE* MATRIX_3D_SPARSE_Shape_Like_Matrix(
    MATRIX_3D_SPARSE* mx_dest,
    const MATRIX_3D_SPARSE* mx_src);

/*! FUNCTION:  MATRIX_3D_SPARSE_GetX_by1D()
 *  SYNOPSIS:  Gets reference to cell in <data> from matrix <smx> at flat index
 * <i_0>. Not designed for public function. This is generally computed by
 * _GetX_byOffset().
 *
 *  RETURN:    Reference to <smx> data array location if success.
 *             Returns NULL if out-of-bounds of sparse matrix.
 */
FLT* MATRIX_3D_SPARSE_GetX_by1D(
    const MATRIX_3D_SPARSE* smx, /* MATRIX_3D_SPARSE object */
    const int i_0);              /* flat index */

/*! FUNCTION:  MATRIX_3D_SPARSE_GetX_byOffset()
 *  SYNOPSIS:  Gets referenct to cell in <data> from matrix <smx> at flat index
 * <i_0>. Not designed for public function. This is generally computed by
 * _GetX_byOffset().
 *
 *  RETURN:    Reference to <smx> data array location if success.
 *             Returns NULL if out-of-bounds of sparse matrix.
 */
FLT* MATRIX_3D_SPARSE_GetX_byOffset(
    const MATRIX_3D_SPARSE* smx, /* MATRIX_3D_SPARSE object */
 ...
 *  SYNOPSIS:  Get reference to cell in corresponding to given (q_0,t_0)
 * coordinates in complete, embedding matrix. Search is linear in the number of
 * discrete data blocks (bounds) per row. Not the fastest for matrix traversal,
 * but good for spot lookups. Caller must call _Index() before calling this.
 *
 *  RETURN:    Reference to <smx> data array location if success.
 *             Returns NULL if out-of-bounds of sparse matrix.
 */
FLT* MATRIX_3D_SPARSE_GetX(
    const MATRIX_3D_SPARSE* smx, /* MATRIX_3D_SPARSE object */
 ...
 */
int MATRIX_3D_SPARSE_GetNumberBounds(
    const MATRIX_3D_SPARSE* smx); /* sparse matrix */

/*! FUNCTION:  EDGEBOUNDS_GetNumberBounds_byRow()
 *  SYNOPSIS:  Gets the number of <bounds> that are on row <id_0>.
 */
int MATRIX_3D_SPARSE_GetNumberBounds_byRow(
    const MATRIX_3D_SPARSE* smx, /* sparse matrix */
 ...
 *             WARNING: Doesn't do safety checks, so make sure to run
 * _GetNumberBounds() to get maximum <r_0> first. Undefined behavior when
 * requesting bound index <r_0> is greater than or equal to number in matrix.
 */
BOUND
MATRIX_3D_SPARSE_GetBound_byIndex(
    const MATRIX_3D_SPARSE* smx, /* sparse matrix */
    const int r_0);              /* IN: bound index in matrix <smx> */

/*! FUNCTION:  MATRIX_3D_SPARSE_Get_CurrentRow_Offset()
 *  SYNOPSIS:  Gets the offset to start of datablock corresponding to <r_0>th
 * bound in <edg_inner>.
 */
int MATRIX_3D_SPARSE_GetOffset_ByIndex_Prv(
    const MATRIX_3D_SPARSE* smx,
 ...
 * bound in <edg_inner>.
 */
int MATRIX_3D_SPARSE_GetOffset_ByIndex_Cur(
    const MATRIX_3D_SPARSE* smx,
    const int r_0); /* index for given bound */
 ...
 * bound in <edg_inner>.
 */
int MATRIX_3D_SPARSE_GetOffset_ByIndex_Nxt(
    const MATRIX_3D_SPARSE* smx,
    const int r_0); /* index for given bound */

/*! FUNCTION:  MATRIX_3D_SPARSE_GetOffset_byCoords()
 *  SYNOPSIS:  Gets the offset to start of datablock corresponding to <r_0>th
 * bound in <edg_inner>. Gets the offsets for the current cell <qx0_prv_out>,
 ...
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetOffset_ByCoords(
    const MATRIX_3D_SPARSE* smx,
...
/*! FUNCTION:  MATRIX_3D_SPARSE_Get_CurrentRow_Offset()
 *  SYNOPSIS:  Gets the offset ind <data> to start of datablock corresponding to
 * <r_0>th bound in <edg_inner>. Gets the offsets for the current cell
 * <qx0_prv_out>, the cell immediately above it on the previous row
 * <qx0_cur_out>, and the cell immediately below it on the next row
 * <qx0_nxt_out>.
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetOffset_ByIndex(
    const MATRIX_3D_SPARSE* smx, /* sparse matrix */
    const int r_0,               /* index for given bound */
 ...
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetOffset_ByIndex_Fwd(
    const MATRIX_3D_SPARSE* smx,
    const int r_0,     /* index for given bound */
    int* qx0_cur_out,  /* OUT: offset to position in current row */
    int* qx0_prv_out); /* OUT: offset to position in previous row */
 ...
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetOffset_ByIndex_Bck(
    const MATRIX_3D_SPARSE* smx,
    const int r_0,     /* index for given bound */
    int* qx0_cur_out,  /* OUT: offset to position in current row */
    int* qx0_nxt_out); /* OUT: offset to position in next row */
 ...
 *             Doesn't do safety checks, so make sure to run
 * _GetNumberBounds_byRow to get maximum <r_0> first. Undefined behavior when
 * requesting bound index <r_0> greater than exists on row.
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetData_byRow_Fwd(
    const MATRIX_3D_SPARSE* smx, /* sparse matrix */
    const int q_0,               /* IN: row/diag id in matrix <smx> */
 ...
 */
STATUS_FLAG
MATRIX_3D_SPARSE_GetData_byRow_Bck(
    const MATRIX_3D_SPARSE* smx, /* sparse matrix */
    const int q_0,               /* IN: row/diag id in matrix <smx> */
...
/*! FUNCTION:  MATRIX_3D_SPARSE_Exp_Embed()
 *  SYNOPSIS:  Embed sparse matrix <smx> into matrix <mx> in exponential scale.
 *             Does not effect sparse matrix.
 *
 *  RETURN:    Pointer to <mx> if success.
 *             Returns NULL if fails.
 */
MATRIX_3D* MATRIX_3D_SPARSE_Exp_Embed(int Q,
                                      int T,
                                      MATRIX_3D_SPARSE* smx, /* sparse matrix */
                                      MATRIX_3D* mx);        /* matrix */
 ...
 *  SYNOPSIS:  Fill all cells (including padding cells) in sparse matrix <smx>
 * with <val>.
 */
void MATRIX_3D_SPARSE_Fill(MATRIX_3D_SPARSE* smx, /* sparse matrix */
                           float val);            /* value to fill data cells with  */

/*! FUNCTION:  MATRIX_3D_SPARSE_Fill()
 *  SYNOPSIS:  Fill all outer padding cells in sparse matrix <smx> with <val>.
 */
void MATRIX_3D_SPARSE_Fill_Outer(
    MATRIX_3D_SPARSE* smx, /* sparse matrix */
 ...
 */
void MATRIX_3D_SPARSE_Fill_Inner(MATRIX_3D_SPARSE* smx, /* sparse matrix */
 ...
 */
void MATRIX_3D_SPARSE_Op(
    MATRIX_3D_SPARSE* mx_out, /* input sparse matrix */
 ...
 */
int MATRIX_3D_SPARSE_Add(
    MATRIX_3D_SPARSE* smx_A,    /* IN: matrix addend */
