 ...
 *  - DESC:    Functions for merging multiple EDGEBOUND objects and
 ...
 *  METHOD:    Going by row/diag <id_0>, by accumulating a sublist of all bounds
 * from <edg_in_1> and <edg_in_2> in <id_0>. Then loops through all bounds in
 * sublist pairwise, merging down overlapping edges. Continues until no more
 * merges can be performed. Pushes the sublist onto <edg_out> and continues as
 * such through each row/diag.
 */
STATUS_FLAG
EDGEBOUNDS_Union_byRow_Old(
    const int Q,          /* query length */
    const int T,          /* target length */
    EDGEBOUNDS* edg_in_1, /* edgebounds (fwd, sorted ascending) */
    EDGEBOUNDS* edg_in_2, /* edgebounds (bck, sorted ascending) */
 ...
 *  SYNOPSIS:  Combine two edgebound lists into one.
 *             Bridges all bounds on each row/diag into single bound, spanning
 * from the min to max index on that row/diag. Assumes input lists are sorted
 * and both have common orientation (byrow/bydiag). WARNING: Sort is not
 * checked.
 */
STATUS_FLAG
EDGEBOUNDS_Union_Abridged(
    const int Q,          /* query length */
    const int T,          /* target length */
    EDGEBOUNDS* edg_in_1, /* edgebounds (fwd, sorted ascending) */
    EDGEBOUNDS* edg_in_2, /* edgebounds (bck, sorted ascending) */
 ...
 *            For each row, looks at each viable antidiag and checks if that
 * antidiag intersects the row. Adds those to growing bound until break is
 * found, then adds it to the row-wise list.
 */
STATUS_FLAG
EDGEBOUNDS_ReorientToRow_byRow(
    const int Q,              /* query length */
    const int T,              /* target length */
    EDGEBOUNDS* edg_in,       /* edgebounds (antidiag-wise, sorted ascending) */
 ...
 *  SYNOPSIS: Reorient EDGEBOUNDS from by-diagonal to by-row.
 *            For each antidiag, look at each cell and integrates it into a
 * growing <edg_rows>.
 */
STATUS_FLAG
EDGEBOUNDS_ReorientToRow_byDiag(
    const int Q,              /* query length */
    const int T,              /* target length */
    EDGEBOUNDS* edg_in,       /* edgebounds (antidiag-wise, sorted ascending) */
    EDGEBOUND_ROWS* edg_rows, /* temporary working space */
 ...
 *  SYNOPSIS: Reorient EDGEBOUNDS from by-diagonal to by-row.
 *            For each antidiagonal look at the disjunctive union to its
 * previous antidiagonal, and only updates those spans at each pass.
 */
STATUS_FLAG
EDGEBOUNDS_ReorientToRow_byDiff(
    const int Q,              /* query length */
    const int T,              /* target length */
    EDGEBOUNDS* edg_in,       /* edgebounds (antidiag-wise, sorted ascending) */
 ...
 */
STATUS_FLAG
EDGEBOUNDS_Reorient_Pushback(
    EDGEBOUNDS* edg, /* edgebounds (row-wise, sorted ascending) */
