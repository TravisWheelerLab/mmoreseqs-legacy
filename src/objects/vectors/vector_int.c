 ...
 */
VECTOR_INT*
VECTOR_INT_WrapArray(INT* array,
                     size_t length,
                     size_t occupied) {
  VECTOR_INT* vec;
  vec = ERROR_malloc(sizeof(VECTOR_INT));
 ...
 */
STATUS_FLAG
VECTOR_INT_Insert(VECTOR_INT* vec,
                  int idx,
                  INT val) {
/* if debugging, do edgebound checks */
#if SAFE
 ...
 */
STATUS_FLAG
VECTOR_INT_Delete(VECTOR_INT* vec,
                  int idx) {
/* if debugging, do edgebound checks */
#if SAFE
  if (idx >= vec->N || idx < 0) {
...
VECTOR_INT_Pop(VECTOR_INT* vec) {
  INT data = VECTOR_INT_Get(vec, vec->N - 1);
  vec->N -= 1;
 ...
 */
inline INT
VECTOR_INT_Popback(VECTOR_INT* vec) {
  INT data = VECTOR_INT_Pop(vec);
 ...
 */
inline int
VECTOR_INT_GetSizeAlloc(VECTOR_INT* vec) {
  return vec->Nalloc;
}
...
VECTOR_INT_Sort(VECTOR_INT* vec) {
  int N = VECTOR_INT_GetSize(vec);
  qsort(vec->data, N, sizeof(INT), INT_CompareTo);
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if ((cmp <= 0) == false) {
        fprintf(stderr, "ERROR: bad sort. %d, %d v %d: %s vs %s\n",
                cmp, i, i + 1, INT_ToString(cur, s_cur), INT_ToString(nxt, s_nxt));
        ERRORCHECK_exit(EXIT_FAILURE);
      }
    }
  }
 ...
 */
inline VECTOR_INT*
VECTOR_INT_Op(VECTOR_INT* vec_out, /* input vector */
              VECTOR_INT* vec_in,  /* output vector (can be input vector) */
              INT (*op)(INT data)) /* unary operation */
{
  int N = VECTOR_INT_GetSize(vec_in);
  if (vec_out == NULL) {
    VECTOR_INT_Create_by_Size(N);
  }
  VECTOR_INT_SetSize(vec_out, N);

  for (int i = 0; i < N; i++) {
    *VECTOR_INT_GetX(vec_out, i) = op(*VECTOR_INT_GetX(vec_in, i));
  }
...
/*! FUNCTION:  VECTOR_INT_Op()
 *  SYNOPSIS:  Perform element-wise binary operation <op>(INT data_1, INT data_2) to each cell in <vec_in_1, vec_in_2> and puts it in <vec_out>.
 ...
 */
inline VECTOR_INT*
VECTOR_INT_BinOp(VECTOR_INT* vec_out,               /* output vector */
                 VECTOR_INT* vec_in_1,              /* first input vector */
                 VECTOR_INT* vec_in_2,              /* second input vector */
                 INT (*op)(INT data_1, INT data_2)) /* binary operation */
{
  int N = VECTOR_INT_GetSize(vec_in_1);
  int N_2 = VECTOR_INT_GetSize(vec_in_2);
  if (N != N_2) {
  ...
  for (int i = 0; i < N; i++) {
    *VECTOR_INT_GetX(vec_out, i) = op(*VECTOR_INT_GetX(vec_in_1, i), *VECTOR_INT_GetX(vec_in_2, i));
  }
 ...
 */
inline STATUS_FLAG VECTOR_INT_Reverse(VECTOR_INT* vec) {
  int N = VECTOR_INT_GetSize(vec);
 ...
 */
STATUS_FLAG
VECTOR_INT_Dump_byOpt(VECTOR_INT* vec,
                      STR delim,
                      STR header,
                      FILE* fp) {
  ...
  fprintf(fp, "[ ");
  for (int i = 0; i < vec->N; i++) {
    fprintf(fp, "%s%s%s", INT_ToString(vec->data[i], s), delim, pad);
  }
  if (vec->N >= 1) {
    fprintf(fp, "%s%s", INT_ToString(vec->data[vec->N - 1], s), pad);
  }
  fprintf(fp, "]\n");
}

/*! FUNCTION:  VECTOR_INT_UnitTest()
