 ...
 */
PTR* VECTOR_PTR_UnwrapArray(VECTOR_PTR* vec,
                            size_t size) {
  PTR* array = vec->data;
  vec = ERROR_malloc(sizeof(VECTOR_PTR));
 ...
 */
int VECTOR_PTR_Search(VECTOR_PTR* vec,
                      PTR val) {
  int N = VECTOR_PTR_GetSize(vec);
  int idx = (N / 2);
  int found = -1;
 ...
 *             Return -1 if <val> is not found.
 */
int VECTOR_PTR_Search_First(VECTOR_PTR* vec,
                            PTR val) {
  int N = VECTOR_PTR_GetSize(vec);
  int idx = (N / 2);
  int found = -1;
 ...
 *             Return -1 if <val> is not found.
 */
int VECTOR_PTR_Search_Last(VECTOR_PTR* vec,
                           PTR val) {
  int N = VECTOR_PTR_GetSize(vec);
  int idx = (N / 2);
  int found = -1;
 ...
 *             neg if <vec_A> < <vec_B>.
 */
int VECTOR_PTR_Compare(VECTOR_PTR* vec_A,
                       VECTOR_PTR* vec_B) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if ((cmp <= 0) == false) {
        fprintf(stderr, "ERROR: bad sort. %d, %d v %d: %s vs %s\n",
                cmp, i, i + 1, PTR_ToString(cur, s_cur), PTR_ToString(nxt, s_nxt));
        ERRORCHECK_exit(EXIT_FAILURE);
      }
    }
  }
 ...
 */
inline STATUS_FLAG VECTOR_PTR_Reverse(VECTOR_PTR* vec) {
  int N = VECTOR_PTR_GetSize(vec);
...
VECTOR_PTR_Dump_byOpt(VECTOR_PTR* vec,
  ...
  fprintf(fp, "[ ");
  for (int i = 0; i < vec->N; i++) {
    fprintf(fp, "%s%s%s", PTR_ToString(vec->data[i], s), delim, pad);
  }
  if (vec->N >= 1) {
    fprintf(fp, "%s%s", PTR_ToString(vec->data[vec->N - 1], s), pad);
  }
  fprintf(fp, "]\n");
}
