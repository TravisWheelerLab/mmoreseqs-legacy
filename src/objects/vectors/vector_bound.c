    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if ((cmp <= 0) == false) {
        fprintf(stderr, "ERROR: bad sort. %d, %d v %d: %s vs %s\n",
                cmp, i, i + 1, BOUND_ToString(cur, s_cur), BOUND_ToString(nxt, s_nxt));
        ERRORCHECK_exit(EXIT_FAILURE);
      }
    }
  }
 ...
 */
inline STATUS_FLAG VECTOR_BOUND_Reverse(VECTOR_BOUND* vec) {
  int N = VECTOR_BOUND_GetSize(vec);
...
VECTOR_BOUND_Dump_byOpt(VECTOR_BOUND* vec,
  ...
  fprintf(fp, "[ ");
  for (int i = 0; i < vec->N; i++) {
    fprintf(fp, "%s%s%s", BOUND_ToString(vec->data[i], s), delim, pad);
  }
  if (vec->N >= 1) {
    fprintf(fp, "%s%s", BOUND_ToString(vec->data[vec->N - 1], s), pad);
  }
  fprintf(fp, "]\n");
}
