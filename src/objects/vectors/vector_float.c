 ...
 */
FLT* VECTOR_FLT_UnwrapArray(VECTOR_FLT* vec,
                            size_t size) {
  FLT* array = vec->data;
  vec = ERROR_malloc(sizeof(VECTOR_FLT));
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if ((cmp <= 0) == false) {
        fprintf(stderr, "ERROR: bad sort. %d, %d v %d: %s vs %s\n",
                cmp, i, i + 1, FLT_ToString(cur, s_cur), FLT_ToString(nxt, s_nxt));
        ERRORCHECK_exit(EXIT_FAILURE);
      }
    }
  }
 ...
 */
inline STATUS_FLAG VECTOR_FLT_Reverse(VECTOR_FLT* vec) {
  int N = VECTOR_FLT_GetSize(vec);
...
VECTOR_FLT_Dump_byOpt(VECTOR_FLT* vec,
  ...
  fprintf(fp, "[ ");
  for (int i = 0; i < vec->N; i++) {
    fprintf(fp, "%s%s%s", FLT_ToString(vec->data[i], s), delim, pad);
  }
  if (vec->N >= 1) {
    fprintf(fp, "%s%s", FLT_ToString(vec->data[vec->N - 1], s), pad);
  }
  fprintf(fp, "]\n");
}
