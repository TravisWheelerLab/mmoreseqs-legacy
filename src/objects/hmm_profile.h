...
/* Destructor */
HMM_PROFILE* HMM_PROFILE_Destroy(HMM_PROFILE* prof);
...
void HMM_PROFILE_SetTextField(char** prof_field, char* text);

/* Set HMM Model Length and allocate memory for nodes */
void HMM_PROFILE_SetModel_Length(HMM_PROFILE* prof, int length);

/* Set alphabet (DNA or AMINO ACID) for HMM_PROFILE */
void HMM_PROFILE_SetAlphabet(HMM_PROFILE* prof, char* alph_name);

/* Determine the consensus sequence (highest likelihood output) using
 * HMM_PROFILE */
...
void HMM_PROFILE_BG_SetLength(HMM_PROFILE* prof, int length);

/* Constrain model to range [tbeg, tend] */
void HMM_PROFILE_SetSubmodel(HMM_PROFILE* prof, int tbeg, int tend);

/* Unconstrain model to cover entire */
void HMM_PROFILE_UnsetSubmodel(HMM_PROFILE* prof);

/* Output HMM_PROFILE to FILE POINTER */
void HMM_PROFILE_Dump(HMM_PROFILE* prof, FILE* fp);
