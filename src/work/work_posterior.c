 ...
 */
void WORK_posterior_sparse(WORKER* worker) {
  ...
  printf_vhi("null1: %f, null2: %f\n",
             worker->result->scores.null1_hmm_bias, worker->result->scores.null2_seq_bias);
 ...
 */
void WORK_null1_hmm_bias(WORKER* worker) {
  FILE* fp = NULL;
 ...
 */
void WORK_decode_domains(WORKER* worker) {
  FILE* fp = NULL;
  TASKS* tasks = worker->tasks;
  ARGS* args = worker->args;
  CLOCK* timer = worker->timer;
 ...
 */
void WORK_decode_posterior(WORKER* worker) {
  FILE* fp = NULL;
  ARGS* args = worker->args;
  TASKS* tasks = worker->tasks;
  CLOCK* timer = worker->timer;
...
#if DEBUG
  {
    ...
    fp = fopen(DEBUG_FOLDER "/my.posterior.sp.exp.000.mx", "w+");
 ...
 */
void WORK_null2_seq_bias(WORKER* worker) {
  FILE* fp = NULL;
  ARGS* args = worker->args;
  TASKS* tasks = worker->tasks;
  CLOCK* timer = worker->timer;
