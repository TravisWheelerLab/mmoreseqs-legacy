 ...
 *  - DESC:    Workflow Subroutines for Posterior Algorithms.
 *******************************************************************************/
 ...
 */
void WORK_posterior_bydom(WORKER* worker) {
 ...
 */
void WORK_posterior_sparse_bydom(WORKER* worker) {
  FILE* fp = NULL;
...
#if DEBUG
      {
        fp = fopen(DEBUG_FOLDER "/my.sparse_fwd.dom.000.mx", "w+");
...
#if DEBUG
      {
        fp = fopen(DEBUG_FOLDER "/my.sparse_bck.dom.000.mx", "w+");
...
#if DEBUG
      {
        fp = fopen(DEBUG_FOLDER "/my.sparse_post.dom.000.mx", "w+");
 ...
 */
void WORK_bound_fwdback_sparse_bydom(WORKER* worker) {
  FILE* fp = NULL;
  ARGS* args = worker->args;
  TASKS* tasks = worker->tasks;
  CLOCK* timer = worker->timer;
 ...
 */
void WORK_null1_hmm_bias_bydom(WORKER* worker) {
  FILE* fp = NULL;
  TASKS* tasks = worker->tasks;
  ARGS* args = worker->args;
  CLOCK* timer = worker->timer;
 ...
 *             Only for domain region.
 */
void WORK_decode_posterior_bydom(WORKER* worker) {
  FILE* fp = NULL;
  ARGS* args = worker->args;
  TASKS* tasks = worker->tasks;
  CLOCK* timer = worker->timer;
 ...
 *             Only for domain region.
 */
void WORK_null2_seq_bias_bydom(WORKER* worker) {
  FILE* fp = NULL;
  ARGS* args = worker->args;
  TASKS* tasks = worker->tasks;
  CLOCK* timer = worker->timer;
