                               ...
                               float* compo_bias)             /* OUTPUT: Null2 composition bias */
{
  ...
  for (q_0 = 1; q_0 <= Q; q_0++) {
    ...
    {
      // if ( q_0 < 10 ) {
      //    printf("SUMMING: q_0 = %d...\n", q_0);
      //    fprintf(stdout, "ST[%3d]: %12.7f %12.7f %12.7f\n", 1,
      //       MX_2D( dom_def->st_freq, 1, M_ST ),
      //       MX_2D( dom_def->st_freq, 1, I_ST ),
      //       MX_2D( dom_def->st_freq, 1, D_ST ) );
      //    fprintf(stdout, "SP[%3d]: %12.7f %12.7f %12.7f %12.7f %12.7f\n", 1,
      //       VEC_X( dom_def->sp_freq, 0 ),
      //       VEC_X( dom_def->sp_freq, 1 ),
      //       VEC_X( dom_def->sp_freq, 2 ),
      //       VEC_X( dom_def->sp_freq, 3 ),
      //       VEC_X( dom_def->sp_freq, 4 ) );
      // }
    }
#endif
    ...
    if (is_q_0_in_dom_range == true || DEBUG) {
...
#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.1.sparse.000.csv", "w+");
    fprintf(fp, "<1>\n");
  ...
  for (t_0 = 0; t_0 <= T; t_0++) {
    ...
    for (st_0 = 0; st_0 < NUM_NORMAL_STATES; st_0++) {
      if (MX_2D(dom_def->st_freq, t_0, st_0) > 0.0) {
        MX_2D(dom_def->st_freq, t_0, st_0) = logf(MX_2D(dom_def->st_freq, t_0, st_0));
      } else {
        MX_2D(dom_def->st_freq, t_0, st_0) = -INF;
      }
      // MATRIX_2D_Logf( dom_def->st_freq );
    }
  }
  /* for each special state emissions */
  for (int st_0 = 0; st_0 < NUM_SPECIAL_STATES; st_0++) {
    if (VEC_X(dom_def->sp_freq, st_0) > 0.0f) {
      VEC_X(dom_def->sp_freq, st_0) = logf(VEC_X(dom_def->sp_freq, st_0));
    } else {
      VEC_X(dom_def->sp_freq, st_0) = -INF;
    }
  }

#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.2.sparse.000.csv", "w+");
    fprintf(fp, "<2>\n");
    fprintf(fp, "==> NORMAL STATES (st_freq) <=\n");
...
#if DEBUG
  {
    fp = fopen(DEBUG_FOLDER "/my.post_vec.3.sparse.000.csv", "w+");
    fprintf(fp, "<3>\n");
    fprintf(fp, "==> NORMAL STATES (st_freq) <=\n");
    for (int t_0 = T_range.beg; t_0 < T_range.end; t_0++) {
      tx0 = t_0 - T_range.beg;
...
#if DEBUG
  {
    fprintf(stdout, "xfactor: %12.7f,  NCJ: %12.7f %12.7f %12.7f\n",
  ...
  {
    // fp = stdout;
    fp = fopen(DEBUG_FOLDER "/my.null2.sparse.000.csv", "w+");
  ...
  *compo_bias = bias;
  dom_def->null2_seq_bias = bias;

#if DEBUG
  {
    // fp = stdout;
    fp = fopen(DEBUG_FOLDER "/my.null2sc.sparse.000.csv", "w+");
