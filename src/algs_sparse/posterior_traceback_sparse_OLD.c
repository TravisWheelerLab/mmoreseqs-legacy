...
Select_Path_from_M(MATRIX_3D_SPARSE* st_SMX,
                   MATRIX_2D* sp_MX,
                   int q_0,
                   int t_0,
                   int st_cur,
 ...
 */
STATUS_FLAG
run_OptimalAccuracy_Traceback_Sparse_OLD(const SEQUENCE* query,         /* query sequence */
                                         ...
                                         ALIGNMENT* aln)                /* OUTPUT: optimal alignment */
{
  ...
  while (st_prv != S_ST) {
    cnt++;
    printf("[%d]: (q_0,t_0) = (%d,%d) | st = %d = %s | sc = %.9f\n",
           cnt, q_0, t_0, st_prv, STATE_NAMES[st_prv], sc_cur);
 ...
 */
STATUS_FLAG
run_OptimalAccuracy_Sparse_OLD(const SEQUENCE* query,         /* query sequence */
                               const HMM_PROFILE* target,     /* target HMM model */
                               const int Q,                   /* query length */
                               const int T,                   /* target length */
                               EDGEBOUNDS* edg,               /* edgebounds */
