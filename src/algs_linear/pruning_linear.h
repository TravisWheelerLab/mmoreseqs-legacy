...
/*! FUNCTION: 	PRUNER_edgetrim_by_global_and_diag_Linear()
 *  SYNOPSIS: 	Prunes antidiagonal of Cloud Search.
 * 				Uses x-drop and only trims in from left and right ends of search
 *space. No bifurcation.
 ...
 * 				(1) Updates the total_max, which stores the highest scoring cell in
 *the matrix thus far. (1b) If diag_max falls below score global threshold,
 *terminate entire search. (2) Performs pruning from left-edge, moving right
 *until a cell is found which all states fall below limit = (total_max - alpha).
...
PRUNER_edgetrim_by_global_and_diag_Linear(
    ...
    const RANGE vit_range,  /* antidiagonal locations for the start-end of the
                               input viterbi alignment */
    const int d_1,          /* previous antidiagonal */
    const int d_0,          /* current antidiagonal */
    const int dx1,          /* previous antidiag (mod-mapped) */
    const int dx0,          /* current antidiag (mod-mapped) */
