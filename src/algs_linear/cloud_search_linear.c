...
STATUS_FLAG run_Cloud_Forward_Linear(
  ...
  int i;        /* antidiagonal, row, column indices */
  int q_0;      /* real index of current and previous rows (query) */
  ...
  float prv_B;               /* previous (B) begin and (E) end states */
  float prv_sum;             /* temp subtotaling vars */
                          ...
                          float* max_sc)             /* OUTPUT: highest score found during search */
{
  ...
  /* vars for recurrence scores */
  float prv_M, prv_I, prv_D;    /* previous (M) match, (I) insert, (D) delete states */
  float prv_E;           /* previous (B) begin and (E) end states */
  float prv_sum;      /* temp subtotaling vars */
  float sc_M, sc_I; /* match, insert, delete, end scores */
